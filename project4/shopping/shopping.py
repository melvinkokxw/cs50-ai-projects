...
def load_data(filename):
    ...
    df = df.astype({"Weekend": "int64", "Revenue": "int64"})

    # Convert object values to int
    replace_obj = {"VisitorType": {"New_Visitor": 0, "Other": 0, "Returning_Visitor": 1},
                   "Month":       {"Jan": 0, "Feb": 1, "Mar": 2, "Apr": 3, "May": 4, "June": 5,
                                   "Jul": 6, "Aug": 7, "Sep": 8, "Oct": 9, "Nov": 10, "Dec": 11}}
    df.replace(replace_obj, inplace=True)

    # Split dataframe into evidence and labels
    evidence = df.drop(["Revenue"], axis=1)
    # labels gets converted to series instead of dataframe
    labels = df["Revenue"]
...
def train_model(evidence, labels):
    ...
    """
    # Use k-nearest neighbour model
    model = KNeighborsClassifier(n_neighbors=1)

    # Fit model with train x and y
...
def evaluate(labels, predictions):
    ...
    """
    # Count actual number of 1s and 0s
